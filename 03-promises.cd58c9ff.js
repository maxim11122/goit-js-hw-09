...
let amount = 0;
let count = 0;
function createPromise(count1, delay) {
    return new Promise((resolve, reject)=>{
...
formEl.addEventListener("submit", (ev)=>{
    ...
    for(let i = 0; i < amount; i += 1){
        setTimeout(()=>{
            count += 1;
            createPromise(count, time).then(({ count: count2 , delay  })=>{
                (0, _notiflixDefault.default).Notify.success(`${emojiOk} Fulfilled promise ${count2} in ${delay}ms`);
            }).catch(({ count: count3 , delay  })=>{
                (0, _notiflixDefault.default).Notify.failure(`${emojiFail} Rejected promise ${count3} in ${delay}ms`);
            });
            time += step;
        }, time);
        count = 0;
