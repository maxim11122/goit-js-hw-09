...
const options = {
    ...
    onClose (selectedDates) {
        if (selectedDates[0] <= new Date()) {
            buttonStartEl.setAttribute("disabled", "disabled");
            (0, _notiflixDefault.default).Report.failure(`Ohhhh...${emoji}`, "Please choose a date in the future", "Alona, try again!!!");
        } else buttonStartEl.removeAttribute("disabled");
        diff = selectedDates[0] - new Date();
    }
};
(0, _flatpickrDefault.default)(inputDataEl, options);
